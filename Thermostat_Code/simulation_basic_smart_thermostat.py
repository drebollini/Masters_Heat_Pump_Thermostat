...
for k in range(0, h):
    ...
    # for j in range(0, h):
    #     heating_curve[k+1] = heat_equ(temp[k], dt, k1, k2, T_out, power*1)
...
plt.title("Basic Smart Thermostat Simulation")
...
hspace=0.2,
wspace=0.2)
plt.savefig("Basic_Smart_Thermostat_Simulation.pdf")
plt.show()
...
plt.title("Basic Smart Thermostat Simulation - Heat Pump Power")
plt.figure(1).set_figwidth(15)
plt.subplots_adjust(top=0.88,
bottom=0.3,
left=0.055,
...
hspace=0.2,
wspace=0.2)
plt.savefig("Basic_Smart_Thermostat_Simulation_Power.pdf")
plt.show()
